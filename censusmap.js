...
Last Update Date: April 9th 2017
****/
...
MapManager.CensusMap = function(cachedDB, mapConfig, rslv) {
	...
	/*points manager for enum address List. To override base methods look at ??.configure*/
	var enumAddressListPtMgr = null;
	...
	this.showEnumALPoints = function() {
		this.removePoints();
        /*shows yahi as a blue dot*/
        omm.setShowUserLocation(true);
		if (!enumAddressListPtMgr) {
			enumAddressListPtMgr = new ESRIMap.GenericPointManagerChild("Block Map");
			if (selectedBCU) enumAddressListPtMgr.setBCU(selectedBCU);
		}
		ENUMAddressList.configure(enumAddressListPtMgr);
		this.show();
		enumAddressListPtMgr.setupPoints();
		enumAddressListPtMgr.plot();
		enumAddressListPtMgr.onShow();
		ESRIMap.ptWrpMgr = enumAddressListPtMgr;
	};
...
MapManager.CensusMap.getCaseData = function() {
    ...
    if (!caseList) {
      caseList = CChlpr.findPage("D_UserWorkListByType.pxResults");
		MobileTestData.collect("D_pyUserWorkList", JSON.stringify(caseList));
    } else {
...
function ENUM_GetOfflineData(cachedFiles, hide, mapConfig) {
	try {
		var promise = new Promise(function(resolve, reject) {
			new MapManager.getMapInstance(cachedFiles, hide, mapConfig).then(
				function(dtS) {
