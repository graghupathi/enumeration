...
/* Page Unload Event */
var pageUnloadEvent = new Event("pageUnload");
...
/*	
 *	Created By: Drake Downs, Edited by Kelsey C. Justis (03-16-2017)
 *	Date: 1.11.17
...
function ValidateCharacterSet (selector) {
  var regex = /^([A-Za-z0-9&'\s\(\),-\.\ÁÉÍÓÚÅÜÑáéíóúüñ/]*)$/;
...
CB.enumGoForward = function() {
  ...
  document.dispatchEvent(pageUnloadEvent);
};
...
CB.enumGoBack = function(){
  ...
  if(pega.mobile.isHybrid) {   
    ...
    } else if(questionName == "ProxyAttempt_QSTN") {
      if(!workPage.hasMessages()) {
        var errorMessage = CB.getAndParseFieldValue("pyWorkPage", "pyCaption", "ProxyAttempt_HARD");
        workPage.addMessage(errorMessage);
        pega.ui.DCUtil.refresh();
      }else {
        workPage.clearMessages();
...
/*
*	Returns localized version of Census Date in String form
*	Does Not append to day (example: April 1,2018)
*	Created by: Kyle Gravel
*/
CB.getLocalizedCensusDate = function() {
  var workPage = pega.ui.ClientCache.find("pyWorkPage");
  var censusDate = workPage.get("CensusDate");
  if (censusDate) {
    censusDate = censusDate.getValue();
    var year = parseInt(censusDate.substring(0,4));
    var month = parseInt(censusDate.substring(4,6));
    var day = parseInt(censusDate.substring(6));
  }
...
CB.addCaseNote = function(caseText,address,redAddressText) {  
  if (pega.mobile.isHybrid) {
   try {  
    if (caseText.trim() == ""){
      return;
    ...
    if (!CaseNotesPage) {
      CaseNotesPage = pega.ui.ClientCache.createPage("caseNotesPage");
    }
    redAddressText = redAddressText || false;
    address = address || pega.ui.ClientCache.find("pyWorkPage.HouseholdRoster.LocationAddress.PartialAddress").getValue();
    ...
    CaseNotesPage.put("Address", address);
    CaseNotesPage.put("RedAddressText", redAddressText);
    caseNotesList.add().adoptJSON(CaseNotesPage.getJSON());

    /*increment case note count, used by CaseDetails screen*/
    var questFlags = pega.ui.ClientCache.find("pyWorkPage.QuestFlags");
    if(!questFlags) {
      alert("Quest flags does not exist");
    }
    var caseNoteCount = caseNotesList.size();
    console.log("Case note count: " + caseNoteCount);
    questFlags.put("CaseNoteCount",caseNoteCount);
    if(caseNoteCount == 0) {
    ...
    /*var count = pega.ui.ClientCache.find("pyWorkPage.QuestFlags.CaseNoteCount").setValue(caseNoteCount);*/
    /*var setPropInClipboard = pega.u.d.setProperty("pyWorkPage.QuestFlags".CaseNoteCount", caseNoteCount);*/
    var workPage = pega.ui.ClientCache.find("pyWorkPage");
    var currentSurveyQuestion = workPage.get("CurrentSurveyQuestion");
    currentSurveyQuestion = currentSurveyQuestion ? currentSurveyQuestion.getValue() : "";
    if(currentSurveyQuestion == "CaseDetails_QSTN") {
      pega.u.d.refreshSection("CaseDetails"); 
    }
    else if(currentSurveyQuestion == "CaseNotes_QSTN") {
      pega.u.d.refreshSection("CaseNotes_INST");
    }
    console.log("quest flags page: " + questFlags.getJSON());
    
    /*Set proper flag if danger*/
