...
ESRIMap.eventListeners = {
  onAction: function(data) {
    ...
    if (data.actionID === "showMap") {
      globalMap.show();
    }
    if (data.actionID === "showhint") {
      globalMap.showHint({
    ...
    if (data.actionID === "precachelayers") {
      MapManager.CensusMap.listCachedFiles().then(function(data) {
        var hide = true;
        var mapConfig = {
          /***
                  Purpose: This is the title shown on top of the map
                      NOTE: 1. It is not supported for Map in the Browser; however test harness browser Map supports it for enabling testing for mobile devices
                            2. On mobile devices lines after <br/> will appear on new line.
                  ***/
          mapTitle: "Pre-cache",
          mapType: "",

        };
        map = ALMCB_GetOfflineData(data, hide, mapConfig);
      });
    }
  },
...
ESRIMap.configureAtlantis = function(cfg, rslv) {
  ...
  try {
    ...
    globalMap.setShowUserLocation(false);
...
ESRIMap.onShow = function(no_zoomFlag) {
  ...
  if ((!cbGlbMap || gblMapType === 'testharness') && ESRIMap.ptWrpMgr) {
    ESRIMap.ptWrpMgr.clearAllPlots();
...
ESRIMap.menuActions = [{
  ...
  displayName: "Show Hint",
  actionID: "showhint"
}, {
  displayName: "Precache Map Layers",
  actionID: "precachelayers"
}];
